...
public class Map extends AppCompatActivity implements GoogleMap.OnMarkerClickListener, OnMapReadyCallback {
            ...
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    if (output != null) {
                        ...
                        site_details.put("name","John");
                        site_details.put("date","07/07/05");
                        site_details.put("location","232 S Bridge St, Christiana, PA 17509");
                        site_details.put("other_questions","blah blah blah");


                        googleMap.addMarker(new MarkerOptions().position(output).title("Location: " + title).icon(BitmapDescriptorFactory.defaultMarker((float)60.0 + rng.nextFloat()*(float)60.0)).alpha(0.7f));
    ...
    Dictionary<String, String> site_details = new Dictionary<String, String>() {
        ...
        public Enumeration<String> elements() {
            return (Enumeration<String>) values;
        }

        @Override
        public String get(Object o) {
            for (int i = 0; i < keys.size(); i++) {
                if (o.equals(keys.get(i))) {
                    return values.get(i);
                }
            }
            return null;
